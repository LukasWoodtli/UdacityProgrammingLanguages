...
def closure (grammar, i, x, ab, cd, j):
    rules = [(entry[0], [], entry[1], i) for entry in grammar if cd != [] and entry[0] == cd[0]]
    return rules
    ...
    for i in range(len(tokens)):
        while True:
            ...
            for state in chart[i]:
                ...
                # where the c's match make a next state c -> . p q r, j
                # ==> We are about to start parsing a "c", but "c" might be
                # something like "exp" with its own production rules.
                # We'll bring those production rules in
                next_states = closure(grammar, i, x, ab, cd, j)
                for next_state in next_states:
                    changes = addtochart(chart, i, next_state) or changes

                # Current state === x -> a b . c d, j
                # Option 2: If tokens[i] = c, make a next state
                # x -> a b c . d, j in chart[i+1]
                # ==> We are looking  for to parse token c next and the
                # current token is exactly c!
                # So we parse over it and move to j+1
                ...
                # for each p -> q . x r, l in chart[j] make a new state
                # p -> q . x r, l in chart[i]
                # ==> We just finished parsin an "x" with this token, but that
                # may have been a sub-step (like matching "exp -> 2" in "2+3").
                # We should update the the higher-level rules as well
